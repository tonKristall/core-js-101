...
function getPolynom(...props) {
  const revProps = props.reverse();
  if (!props) return null;
  return (x) => revProps.reduce((acc, el, index) => acc + el * (x ** index), 0);
}
...
function logger(func, logFunc) {
  return (...props) => {
    const args = JSON.stringify(props).slice(1, -1);
    const start = `${func.name}(${args}) starts`;
    const end = `${func.name}(${args}) ends`;
    logFunc(start);
